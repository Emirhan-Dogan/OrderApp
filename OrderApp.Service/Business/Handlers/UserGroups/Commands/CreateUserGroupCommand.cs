...
namespace Business.Handlers.UserGroups.Commands
{
    public class CreateUserGroupCommand : IRequest<IResult>
    {
        ...
        public class CreateUserGroupCommandHandler : IRequestHandler<CreateUserGroupCommand, IResult>
        {
            ...
            {
                //var isThereStorageRecord = _userGroupRepository.Query().Any(u => u.GroupId == request.GroupId && u.UserId == request.UserId);

                //if (isThereStorageRecord == true)
                //    return new ErrorResult(Messages.NameAlreadyExist);
                var userGroups = await _userGroupRepository.GetListAsync(p => p.UserId == request.UserId);
                foreach (var item in userGroups)
                {
                    _userGroupRepository.Delete(item);
                }
