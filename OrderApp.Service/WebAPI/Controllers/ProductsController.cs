...
namespace WebAPI.Controllers
{
    ...
    public class ProductsController : BaseApiController
    {
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        [HttpGet("getall-page")]
        public async Task<IActionResult> GetListWithPagination([FromQuery]int page)
        {
            var result = await Mediator.Send(new GetProductsWithPaginationQuery { Page=page});
            if (result.Success)
            {
                return Ok(result.Data);
            }
        ...
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(IEnumerable<Product>))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        [HttpGet("getall-storage")]
        public async Task<IActionResult> GetListForStorage(int StorageId)
        {
            var result = await Mediator.Send(new GetProductForStorageQuery { StorageId=StorageId});
            if (result.Success)
            {
                return Ok(result.Data);
            }
